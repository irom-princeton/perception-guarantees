...
thres = 0.3# 0.23
n_exp = f'udf_{thres}_0.01_shift_filter_top'
...
def run_step(env, task, x, y, step, visualize=True):
...
def project_pc(pred_points, pred_rgb):
    ...
    if good_points.sum() != 0:
        ...
        mask = (points[:, 1] > 0.3) & (points[:, 1] < 0.5) 
        points = points[mask]
...
def pred_numcc(args, observation):
    ...
    pred_points, pred_colors, seen_xyz, prefix = run_viz_udf(model, samples, args.device, args)
    cam_position = observation[2]
    ...
    # pc_for_occ = seen_points
    good_points = pc_for_occ[:, 0] != -100

    if good_points.sum() != 0:
        # filter out ceiling and floor
        mask = (pc_for_occ[:, 1] > 0 ) & (pc_for_occ[:, 1] < 1)
    ...
    fig = px.imshow(grid)
    fig.show()
    # plt.savefig(prefix + '_grid.png')

    
    # fig1 = project_pc(pred_points, pred_colors)
    # plt.savefig(prefix + '_proj.png')

    # # visualize all_points
    # fig = plt.figure()
    # ax = fig.add_subplot(111, projection='3d')
    # ax.scatter(all_points[:, 0], all_points[:, 1], all_points[:, 2])
    # plt.show()
    return all_points, prefix
   

def main(numcc_args, task):
    ...
    # run step
    task, pc = run_step(env, task, x, y, 0, visualize=visualize)
    ...
    all_points, prefix = pred_numcc(numcc_args, observation)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--save_dataset', default='/home/zm2074/Projects/data/perception-guarantees/task_scene.npz',
        nargs='?', help='path to save the task files'
    )
    ...
    task.init_state = [x[0], y[0], 0.0, 0.0]  # x, y, vx, vy
    visualize = False

    main(numcc_args, task)
